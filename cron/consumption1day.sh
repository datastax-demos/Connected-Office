...
dse shark <<EOF
INSERT INTO TABLE coffice.consumption_1day
SELECT
    sensor_id,
    consumption,
    consumption_time,
...
FROM (
    SELECT
        sensor_id,
        consumption,
        UNIX_TIMESTAMP(CONCAT(CONCAT(SUBSTR(max_rollup_ts, 0, 10), ' '),
        SUBSTR(max_rollup_ts, 12, 8))) - UNIX_TIMESTAMP(CONCAT(CONCAT(SUBSTR(min_rollup_ts, 0, 10), ' '),
        SUBSTR(min_rollup_ts, 12, 8))) AS consumption_time
    FROM (
        SELECT
            sensor_id,
            ...
            FROM (
                ...
                FROM (
                    ...
                    FROM (
                        ...
                        AND 1 > DATEDIFF(FROM_UNIXTIME(unix_timestamp()), CONCAT(CONCAT(SUBSTR(rollup_ts, 0, 10), ' '), SUBSTR(rollup_ts, 12, 8)))
                    ) p1
                ) p2
            ) dweight
        ) q1
